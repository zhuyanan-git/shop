...
var refreshItem = function(){
	...
	var target = $('.Qfant_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(){
	...
	if($.common.isNotEmpty(panelUrl)){
		...
		$('.mainContent .Qfant_iframe', topWindow).each(function() {
            if ($(this).data('id') == panelUrl) {
                $(this).show().siblings('.Qfant_iframe').hide();
                return false;
            }
		});
	}
...
function createMenuItem(dataUrl, menuName) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Qfant_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.Qfant_iframe').hide().parents('.mainContent').append(str1);
